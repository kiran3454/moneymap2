...
def index():
    if 'username' in session:
        ...
        total_card = sum(transaction[2] for transaction in transactions if transaction[6] == 'cards')
        
        conn.close()

        return render_template('index.html', username=username, total_amount=total_amount, total_upi=total_upi, total_cash=total_cash, total_card=total_card)
...
def edit_transaction():
    if 'username' in session:
        ...
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("UPDATE transactions SET date = ?, category = ?, amount = ?, payment_method = ?, description = ? WHERE id = ?",
                  (date, category, amount, payment_method, description, transaction_id))
        conn.commit()
        conn.close()
...
def daily_spending_data():
    if 'username' in session:
        ...
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("SELECT date, SUM(amount) FROM transactions WHERE user_id = ? GROUP BY date ORDER BY date DESC", (user_id,))
        data = c.fetchall()
        conn.close()
...
def monthly_spending_data():
    if 'username' in session:
        ...
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("SELECT strftime('%Y-%m', date) AS month, SUM(amount) FROM transactions WHERE user_id = ? GROUP BY month ORDER BY month DESC", (user_id,))
        data = c.fetchall()
        conn.close()
...
@app.route('/monthly_breakdown_data')
def monthly_breakdown_data():
    if 'username' in session:
        user_id = session['user_id']
        
        # Fetch monthly breakdown data from the database
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("SELECT category, SUM(amount) FROM transactions WHERE user_id = ? AND strftime('%Y-%m', date) = strftime('%Y-%m', 'now') GROUP BY category", (user_id,))
        data = c.fetchall()
        conn.close()
...
def download_pdf():
    if 'username' in session:
        ...
        path_wkhtmltopdf = r'C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe'  # Update this path as per your installation
